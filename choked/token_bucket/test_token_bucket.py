...
async def test_token_bucket_limits_requests():
    async with RedisTokenBucket(f"tb-{uuid.uuid4()}", request_capacity=3, request_refill_rate=1.0, token_capacity=0, token_refill_rate=0.0) as bucket:
...
async def test_token_bucket_partial_refill():
    async with RedisTokenBucket(f"tb-refill-{uuid.uuid4()}", request_capacity=1, request_refill_rate=1.0, token_capacity=0, token_refill_rate=0.0) as bucket:
        assert await bucket.acquire(requests_needed=1, tokens_needed=0) == True
        assert await bucket.acquire(requests_needed=1, tokens_needed=0) == False
...
async def test_token_bucket_full_refill():
    async with RedisTokenBucket(f"tb-refill-{uuid.uuid4()}", request_capacity=5, request_refill_rate=1.0, token_capacity=0, token_refill_rate=0.0) as bucket:
        for _ in range(5):
            assert await bucket.acquire(requests_needed=1, tokens_needed=0) == True
...
async def test_dual_rate_limiting():
    ...
    async with RedisTokenBucket(f"tb-dual-{uuid.uuid4()}", request_capacity=5, request_refill_rate=1.0, token_capacity=10, token_refill_rate=1.0) as bucket:
